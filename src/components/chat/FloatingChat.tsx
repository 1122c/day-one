    ...
    <div className="fixed bottom-4 right-4 z-50 w-full h-full sm:w-[500px] sm:h-[700px] sm:bottom-4 sm:right-4 bg-white rounded-lg shadow-2xl border border-gray-200 flex flex-col">
      ...
      <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-t-lg">
        ...
        <div className="flex items-center space-x-1 sm:space-x-2">
          <button
            onClick={onMinimize}
            className="p-2 sm:p-1 hover:bg-white/20 rounded transition-colors duration-200"
            title="Minimize"
          >
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-3 sm:space-y-4 bg-gray-50">
        {messages.map((message) => (
          ...
          >
            <div
              className={`max-w-[85%] sm:max-w-sm px-3 py-2 sm:px-4 sm:py-3 rounded-lg ${
                message.sender === 'user'
      ...
      {currentUser && (
        <div className="px-3 sm:px-6 py-2 sm:py-3 border-t border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
          <button
            onClick={() => setShowAISuggestions(!showAISuggestions)}
            className={`mx-auto block text-center font-medium py-2 px-4 sm:px-6 rounded-md transition-all duration-200 ${
              showAISuggestions 
      ...
      {showAISuggestions && currentUser && (
        <div className="border-t border-indigo-200 bg-white max-h-48 sm:max-h-96 overflow-y-auto">
      ...
      <div className="p-3 sm:p-4 border-t border-gray-200 bg-white rounded-b-lg">
        <div className="flex space-x-2">
          ...
          <button
            ...
            className="px-3 sm:px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200 flex items-center min-w-[44px]"
          >
